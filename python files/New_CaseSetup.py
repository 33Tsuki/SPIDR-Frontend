...
class StatusCard(QFrame):
    def __init__(self, title, status_text, description, status_color, icon_text="✓"):
        ...
        self.setStyleSheet(f"""
            StatusCard {{
                {ModernDarkTheme.status_card_success_style()}
                padding: 12px;
            }}
        """)
...
class ActionButton(QFrame):
    def __init__(self, icon_text, label_text):
        ...
        self.setFrameStyle(QFrame.Shape.StyledPanel)
        self.setStyleSheet(f"""
            {ModernDarkTheme.action_button_style()}
        """)
...
class ThermoMedAnalytics(QMainWindow):
    def __init__(self):
        ...
        self.case_num_input.setPlaceholderText("e.g. 10245")
        self.case_num_input.setStyleSheet("""
            QLineEdit {
                padding: 10px;
        ...
        """)
        case_info.content_layout.addWidget(self.case_num_input)
        ...
        self.target_combo.addItem("🔍 Select organ type...")
        self.target_combo.addItems(["Heart", "Liver", "Kidney", "Lung", "Brain"])
        ...
        """)
        case_info.content_layout.addWidget(self.target_combo)
    ...
    def open_input_selection(self):
        ...
        case_num = self.case_num_input.text().strip()
        target = self.target_combo.currentText()
        
        if not case_num:
            QMessageBox.warning(self, "Validation Error", "Please enter a Case Number.")
            return
            
        if target == "🔍 Select organ type...":
            QMessageBox.warning(self, "Validation Error", "Please select a Target Organ.")
            return

        # Save to database
        if self.db.add_case(case_num, target):
            from Input_Selection import ThermalSegStage1
            self.input_selection_window = ThermalSegStage1(case_number=case_num)
            self.input_selection_window.show()
            self.close()
        else:
             QMessageBox.warning(self, "Database Error", f"Case {case_num} already exists or could not be added.")
