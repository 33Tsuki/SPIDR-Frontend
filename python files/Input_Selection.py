...
from Thermal_Data_Processing import MedThermoStage2
    ...
    def __init__(self, number, title, subtitle, status="pending"):
        ...
        if status == "completed":
            ...
            self.circle.setStyleSheet(f"""
                background-color: {ModernDarkTheme.SUCCESS};
                color: white;
                border-radius: 18px;
                font-size: 18px;
                font-weight: bold;
...
class ConfigPanel(QFrame):
    run_requested = pyqtSignal(str, str) # Signals seg_path, img_path
    ...
    def __init__(self):
        ...
        # Run button
        self.run_btn = QPushButton("Run Stage 1 – Segmentation ▶")
    ...
    def validate_and_run(self):
        seg_path = self.seg_input.text().strip()
        img_path = self.img_input.text().strip()
        
        if seg_path and img_path:
             self.run_requested.emit(seg_path, img_path)
...
class ThermalSegStage1(QMainWindow):
    def __init__(self, case_number=None):
        ...
        display_case = f"Case #{self.case_number}" if self.case_number else "New Case"
        current = QLabel(f"{display_case} - Stage 1")
        ...
        config_panel.run_requested.connect(self.run_analysis)
    ...
    def run_analysis(self, seg_path, img_path):
        """Execute the analysis process (Simulated)"""
        # Disable run button
        self.config_panel.run_btn.setEnabled(False)
        self.config_panel.run_btn.setText("Processing... ⏳")
        
        # Reset execution panel
        self.exec_panel.set_step_status(1, "pending")
        self.exec_panel.set_step_status(2, "pending")
        self.exec_panel.set_step_status(3, "pending")
        self.exec_panel.set_output_value("Generated Findings", "—")
        self.exec_panel.set_output_value("Output Path", "—")
        
        # Start simulation sequence
        # Step 1: Input Validation
        self.exec_panel.set_step_status(1, "running")
        
        # Use QTimer to sequence the steps (Simulating async work)
        QTimer.singleShot(1000, lambda: self.finish_step_1())

    def finish_step_1(self):
        # Update database with file paths
        if self.case_number:
            seg_path = self.config_panel.seg_input.text().strip()
            img_path = self.config_panel.img_input.text().strip()
            self.db.update_case_files(self.case_number, seg_path, img_path)

        self.exec_panel.set_step_status(1, "completed")
        self.exec_panel.set_step_status(2, "running")
        
        # Step 2: Processing (longer)
        QTimer.singleShot(3000, lambda: self.finish_step_2())
        
    def finish_step_2(self):
        self.exec_panel.set_step_status(2, "completed")
        self.exec_panel.set_step_status(3, "running")
        
        # Step 3: JSON Generation
        QTimer.singleShot(1000, lambda: self.finish_step_3())
        
    def finish_step_3(self):
        self.exec_panel.set_step_status(3, "completed")
        
        # Update outputs
        import random
        findings = f"{random.randint(3, 12)} regions detected"
        self.exec_panel.set_output_value("Generated Findings", findings)
        self.exec_panel.set_output_value("Output Path", "/local/data/output/case_492.json")
        
        # Re-enable button
        self.config_panel.run_btn.setEnabled(True)
        self.config_panel.run_btn.setText("Run Stage 1 – Segmentation ▶")
        
        # Transition to Stage 2
        self.stage2_window = MedThermoStage2()
        self.stage2_window.show()
        self.close()
    ...
    def show_history(self):
        """Show the history of cases in a dialog."""
        dialog = QDialog(self)
        dialog.setWindowTitle("Case History")
        dialog.resize(800, 500)
        dialog.setStyleSheet(ModernDarkTheme.get_stylesheet())
        ...
        table.setHorizontalHeaderLabels(["Case Number", "Target Organ", "Timestamp", "Status", "SEG File"])
        ...
        for i, case in enumerate(cases):
            table.setItem(i, 0, QTableWidgetItem(str(case['case_number'])))
            table.setItem(i, 1, QTableWidgetItem(str(case['target_organ'])))
            table.setItem(i, 2, QTableWidgetItem(str(case['timestamp'])))
            table.setItem(i, 3, QTableWidgetItem(str(case['status'])))
            table.setItem(i, 4, QTableWidgetItem(str(case['seg_path']) if case['seg_path'] else "-"))
